...
The completed Cu/Ti milestone was pushed as `631d28b`. The follow-up branch is `experiment/002-independent-material-studies`: 18 source-separated studies, three seeds each. Keep the repository private until this round and its publication checks finish. `publish_material_studies.py` implements the owner-authorized second result push and public visibility change; it refuses incomplete fits, changed hashes, nonfinite metrics, broken report links, or failed tests.
